...
Continuous sizing variant: spy_weight = (composite + 5) / 10
Daily rebalance (gap=1).

From sweep: SCORE=0.743, SR=0.743, DD=-4.6%, 250 trades, TO=2.1x
"""
...
def download_regime_data(start="2005-01-01"):
    ...
    if cache_file.exists():
        print("[s525] Loading regime data from cache …", file=sys.stderr)
        return pd.read_pickle(cache_file)

    print("[s525] Downloading all regime data …", file=sys.stderr)
...
def make_signal(regime_df, rebalance_gap=1):
    """Continuous sizing factory. spy_weight = (composite + 5) / 10."""
    def regime_signal(train_px, test_px, **kw):
        ...
        for i, date in enumerate(test_px.index):
            ...
            spy_w = min(max((score + 5.0) / 10.0, 0.0), 1.0)
...
def main():
    ...
    print("s525 — MRDv3 Continuous Sizing", file=sys.stderr)
    print("=" * 60, file=sys.stderr)

    data = download_regime_data()
    regime_df = compute_regime_signals(data)
    print(f"[s525] Composite: mean={regime_df['composite'].mean():.2f}, "
          f"std={regime_df['composite'].std():.2f}", file=sys.stderr)

    prices = download_data(["SPY", "TLT"], start="2010-01-01", asset_class="etf")

    # Run with continuous sizing, daily rebalance
    signal_fn = make_signal(regime_df, rebalance_gap=1)
    ...
    print(f"\n{'='*60}", file=sys.stderr)
    print("s525 — Continuous Sizing (gap=1d)", file=sys.stderr)
    print('='*60, file=sys.stderr)
    ...
    print(f"\nLEDGER_ROW\ts525\tregime-detection\tspy-tlt\t"
          f"{metrics.get('SCORE',0)}\t{metrics.get('sharpe_net',0)}\t"
          f"{metrics.get('max_dd_pct',0)}\t{metrics.get('ann_turnover',0)}\t"
          f"1\t{'keep' if score > 0 else 'discard'}\t"
          f"MRDv3 continuous sizing: spy_w=(composite+5)/10, daily rebalance")
